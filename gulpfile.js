...
gulp.task("prod", () => {
    ...
    gulp.src("./src/main.js")
        ...
        .pipe(gulp.dest(prod));
    return gulp.src("./scss/style.scss")
        .pipe(sass().on("error", sass.logError)) // 1. Обрабатываем SASS
        .pipe(postcss([autoprefixer()]))  // 2. Добавляем префиксы
        .pipe(cleanCSS())                        // 3. Сжимаем CSS
        .pipe(gulp.dest(prod));                  // 4. Перемещаем
});
